    ...
    requestGraph: function() {
      ...
      var jsonRequest = new Request.JSON({
        ...
        onSuccess: function(json) {
          ...
          that.childResponsesRemaining = json.children.length;
          ...
          json.children.each( function(child) {
            ...
            var jsonRequest = new Request.JSON({
              ...
              onSuccess: function(responseJSON, responseText) {
                ...
                json.children.each( function(child) {
                  if (child.name == responseJSON.name) {
                    // Subchilds einhaengen
                    child.children = responseJSON.children;
                    child.children.done = true;
                  }
                });
                json.children[child.name] = responseJSON.children;
                that.registerChildResponse(that, json, id);
              }
            }).get();
          });
...
window.addEvent('domready', function() {
  Layout.init();
  init();
  PageController.makeRequestAndPlot('ChristophTrautwein');
});
